...
export class TelemetryManager {
    ...
    private updateConnectionStatus(history: { value: number }[]) {
        ...
        const avgLatency = Math.round(history.reduce((sum, h) => sum + h.value, 0) / history.length);
        const prevDuration = history.length > 1 ? history[history.length - 2].value : null;
        ...
        const points = latencies.map((val, i) => {
            const x = latencies.length > 1 ? (i / (latencies.length - 1)) * svgWidth : 0;
