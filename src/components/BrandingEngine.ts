...
const DEFAULT_BRANDING_VALUES = {
  app: {
    ...
    shortName: "DoR", // New: For apple-mobile-web-app-title and application-name
    subtitle:
      "Official Department of Roads Weekly Progress & Statistics Dashboard.", // New: For meta description
  },
  seo: {
    description:
      "Official Department of Roads Weekly Progress & Statistics Dashboard.",
    ogImage: "/logo.png",
    msTileColor: "#1a5c3a", // New: Default for Windows tile
    msTileImage: "/logo.png", // New: Default for Windows tile
    iosSplashScreens: [
      {
        href: "/logo.png",
        media:
  ...
  .object({
    app: z
      .object({
        ...
        shortName: z.string().optional(), // New
    ...
    colors: z
      .object({
        ...
        msTileColor: z.string().optional(), // New
        msTileImage: z.string().optional(), // New
        ...
        warningBorder: z.string().optional(),
      })
      .optional(),
    organization: z
      .object({
...
export class BrandingEngine {
  private static _listeners = new Set<
    (config: z.infer<typeof BrandingSchema>) => void
  >();

  private static _config: z.infer<typeof BrandingSchema> = new Proxy(
  ...
  private static pendingThemeColor: string =
    DEFAULT_BRANDING_VALUES.colors.primary;
  private static pendingDarkColor: string =
    DEFAULT_BRANDING_VALUES.colors.backgroundDark;
  ...
  static getFullTitle(suffix = "") {
    const base =
      this._config.app?.fullName || DEFAULT_BRANDING_VALUES.app.fullName;
    return suffix ? `${base} | ${suffix}` : base;
  }

  static getMetaDescription() {
    return (
      this._config.seo?.description || DEFAULT_BRANDING_VALUES.seo.description
    );
  }
  ...
  static async apply() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (parsed.success) {
          ...
          branding = {
            ...
            colors: {
              ...DEFAULT_BRANDING_VALUES.colors,
              ...parsed.data.colors,
            },
            organization: {
              ...DEFAULT_BRANDING_VALUES.organization,
              ...parsed.data.organization,
            },
            report: {
              ...DEFAULT_BRANDING_VALUES.report,
              ...parsed.data.report,
            },
            lastUpdate: {
              ...DEFAULT_BRANDING_VALUES.lastUpdate,
              ...parsed.data.lastUpdate,
            },
          };
        } else {
          console.warn(
            "[BrandingEngine] branding.json validation failed, using defaults:",
            parsed.error.format(),
          );
      ...
      console.warn(
        "[BrandingEngine] Failed to fetch branding.json, using defaults:",
        e,
      );
    }
  }
  ...
  private static updateDOM(branding: z.infer<typeof BrandingSchema>) {
    // 1. App Identity
    document.title = branding.app?.title || DEFAULT_BRANDING_VALUES.app.title;
    ...
    this.updateMeta(
      ...
      branding.seo?.description ||
        branding.app?.subtitle ||
        DEFAULT_BRANDING_VALUES.seo.description,
    );
    ...
    this.updateMeta(
      ...
      branding.app?.shortName ||
        branding.app?.title ||
        DEFAULT_BRANDING_VALUES.app.shortName,
    );
    this.updateMeta(
      "application-name",
      branding.app?.shortName ||
    ...
    this.updateMeta(
      ...
      branding.colors?.msTileColor ||
        branding.colors?.primary ||
        DEFAULT_BRANDING_VALUES.seo.msTileColor,
    );
    this.updateMeta(
      "msapplication-TileImage",
      branding.colors?.msTileImage ||
        branding.seo?.ogImage ||
        DEFAULT_BRANDING_VALUES.seo.msTileImage,
    );
    this.pendingFaviconUrl = branding.seo?.ogImage || "/logo.png";
    this.preLoadImage(this.pendingFaviconUrl);

    // 2.1 iOS Splash Screens
    // Remove existing apple-touch-startup-image links to avoid duplicates
    document.head
      .querySelectorAll('link[rel="apple-touch-startup-image"]')
      .forEach((link) => link.remove());
    const splashScreens =
      branding.seo?.iosSplashScreens ||
      DEFAULT_BRANDING_VALUES.seo.iosSplashScreens;
    splashScreens.forEach((splash) => {
      this.updateLink("apple-touch-startup-image", splash.href, splash.media);
    });

    // 3. Theme Colors
    if (branding.colors?.primary)
      this.pendingThemeColor = branding.colors.primary;
    if (branding.colors?.backgroundDark)
      this.pendingDarkColor = branding.colors.backgroundDark;

    // Sync Chart Variables
    const root = document.documentElement;
    if (branding.colors?.good)
      root.style.setProperty("--good", branding.colors.good);
    if (branding.colors?.stable)
      root.style.setProperty("--stable", branding.colors.stable);
    if (branding.colors?.critical)
      root.style.setProperty("--critical", branding.colors.critical);
    if (branding.colors?.warning)
      root.style.setProperty("--warning", branding.colors.warning);
    if (branding.colors?.warningBg)
      root.style.setProperty("--warning-bg", branding.colors.warningBg);
    if (branding.colors?.warningText)
      root.style.setProperty("--warning-text", branding.colors.warningText);
    if (branding.colors?.warningBorder)
      root.style.setProperty("--warning-border", branding.colors.warningBorder);
  ...
  public static finalizeThemeColor() {
    this.updateMeta("theme-color", this.pendingThemeColor);
    this.updateMeta(
      "theme-color",
      this.pendingThemeColor,
      false,
      "(prefers-color-scheme: light)",
    );
    this.updateMeta(
      "theme-color",
      this.pendingDarkColor,
      false,
