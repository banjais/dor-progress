    ...
    async function checkFreshInstall() {
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (backups.length > 0) {
            const confirmRestore = confirm(currentLang === 'en'
    ...
    async function setupSecurity() {
      try {
        ...
        addToast("error", currentLang === 'en' ? "Security init failed" : "प्रणाली असफल");
    ...
    async function authenticatedFetch(path, options = {}, maxRetries = 3) {
      const url = path.startsWith('http') ? path : `${WORKER_BASE}${path.startsWith('/') ? '' : '/'}${path}`;
      const { getToken } = await import("https://www.gstatic.com/firebasejs/11.0.1/firebase-app-check.js");
      ...
      headers['X-Low-Data'] = localStorage.getItem('low-data') === 'true' ? 'true' : 'false';
        ...
        try {
          if (window.appCheck) {
            ...
            if (tokenResult && tokenResult.token) {
              ...
              console.log("[Security] App Check token attached to request:", tokenResult.token.substring(0, 10) + "..."); // Log partial token for privacy
            } else {
          ...
          if (response.ok) {
            ...
            if (isStale) {
              ...
              addToast("info", currentLang === 'en' ? "Using cache" : "क्यास प्रयोग");
            }
            return response;
          }
          ...
          throw new Error(`Attempt ${attempt + 1} failed with status ${response.status}`);
        } catch (err) {
          const isLastAttempt = attempt === effectiveRetries - 1;
          if (isLastAttempt) {
            const errorMsg = currentLang === 'en' ? "Connection failed after multiple attempts." : "धेरै प्रयास पछि जडान असफल भयो।";
    ...
    async function checkStatus() {
      ...
      try {
        ...
        if (res.ok) {
          ...
          addToast("success", currentLang === 'en' ? `Pong! ${duration}ms` : `पङ्! ${duration}ms`);
        } else {
          throw new Error();
        }
      } catch (e) {
    ...
    window.updateVolume = (val) => {
      ...
      targets.forEach(id => {
        ...
        if (el) { el.innerText = icon; if (id === 'header-mute-btn') el.classList.toggle('audio-active', uiVolume > 0); }
      });
    ...
    window.setSoundPack = (pack) => {
      ...
      document.querySelectorAll('.pack-opt').forEach(opt => opt.classList.toggle('active', opt.dataset.pack === pack));
      playClickSound();
    };

    window.toggleMute = () => {
      playClickSound();
      if (uiVolume > 0) { lastVolume = uiVolume; updateVolume(0); }
      else { updateVolume(lastVolume); }
    ...
    window.resetAudioToDefault = () => {
      ...
      addToast("success", currentLang === 'en' ? "Audio settings reset to default." : "ध्वनि सेटिङहरू रिसेट गरियो।");
    };
    ...
    const playSound = (id, checkMute = true) => {
      ...
      try {
        ...
        const audioCtx = window._audioCtx || (window._audioCtx = new (window.AudioContext || window.webkitAudioContext)());
        if (audioCtx.state === 'suspended') audioCtx.resume();
        const osc = audioCtx.createOscillator();
        const gain = audioCtx.createGain();
        osc.type = p.type;
        osc.frequency.setValueAtTime(p.f1 * uiPitch, audioCtx.currentTime);
        if (p.f2) osc.frequency.exponentialRampToValueAtTime(p.f2 * uiPitch, audioCtx.currentTime + p.d);
        gain.gain.setValueAtTime(p.g * (checkMute ? uiVolume : 1), audioCtx.currentTime);
        gain.gain.exponentialRampToValueAtTime(0.0001, audioCtx.currentTime + p.d);
        osc.connect(gain); gain.connect(audioCtx.destination);
        osc.start(); osc.stop(audioCtx.currentTime + p.d);
      } catch (e) { }
    };
    ...
    function getRelativeTimeString() {
      ...
      if (diff < 60) return (currentLang === 'ne' ? toNepaliNumerals(diff) : diff) + " " + t.secsAgo;
      const mins = Math.floor(diff / 60);
      return (currentLang === 'ne' ? toNepaliNumerals(mins) : mins) + " " + t.minsAgo;
    }
    ...
    function toArabicNumerals(str) {
      const n = { "०": "0", "१": "1", "२": "2", "३": "3", "४": "4", "५": "5", "६": "6", "७": "7", "८": "8", "९": "9" };
    ...
    function startVoiceSearch() {
      ...
      if (!SpeechRecognition) {
        addToast("error", currentLang === 'en' ? "Voice search not supported in this browser." : "यो ब्राउजरमा भ्वाइस सर्च समर्थित छैन।");
        return;
      }
      ...
      recognition.onresult = (event) => {
        ...
        handleSearch();
        addToast("info", (currentLang === 'en' ? "Search: " : "खोज: ") + transcript);
    ...
    async function translateAiBrief() {
      ...
      try {
        ...
        if (json.aiSummary && json.aiSummary.brief) {
          typeText(document.getElementById("ai-brief-text"), json.aiSummary.brief, true);
        }
      } catch (e) {
    ...
    async function fetchAiBriefBlob() {
      ...
      const res = await authenticatedFetch(`/api/tts?lang=${currentLang}&quality=${isPremium ? 'premium' : 'standard'}&text=${encodeURIComponent(text)}`);
    ...
    window.toggleReadAloud = () => {
      ...
      if (speechSynth.speaking && !speechSynth.paused) {
        ...
        return;
      } else if (speechSynth.speaking) { // If speaking but not paused (e.g., just started)
        speechSynth.cancel();
      ...
      if (savedVoiceUri) {
        ...
        currentUtterance.voice = voices.find(v => v.voiceURI === savedVoiceUri) || null;
      }

      currentUtterance.rate = parseFloat(localStorage.getItem('tts-rate') || '0.95');
      currentUtterance.pitch = parseFloat(localStorage.getItem('tts-pitch') || '1.0');
      ...
      words.forEach(w => {
        if (w.trim().length > 0) {
          ...
          spanMap.push({ start: cumulativeIdx, end: cumulativeIdx + w.length, el: span });
        } else {
      ...
      currentUtterance.onboundary = (event) => {
        ...
        const match = spanMap.find(m => event.charIndex >= m.start && event.charIndex < m.end);
        if (match) {
    ...
    function showDiagnostics() {
      ...
      const t = I18N[currentLang];
      const dispCount = currentLang === 'ne' ? toNepaliNumerals(criticalRows.length) : criticalRows.length;
    ...
    async function exportHealthReport() {
      ...
      const formattedDate = currentLang === 'en' ? `${bsMonthName} ${displayYear} BS` : `${bsMonthName} ${displayYear} वि.सं.`;
      ...
      reportTitleEl.innerText = (currentLang === 'en' ? `Monthly Health Audit - ${formattedDate}` : `मासिक स्वास्थ्य लेखापरीक्षण - ${formattedDate}`);
      ...
      try {
        const res = await authenticatedFetch(`/api/summary?type=monthly&year=${year}&month=${month}`);
        const json = await res.json();

        // Calculate Risk Score for the Audit
        const totalRows = json.rows.length;
        const criticalCount = json.rows.filter(r => r._status === 'critical').length;
        const auditRiskScore = totalRows > 0 ? Math.round((criticalCount / totalRows) * 100) : 0;
        const riskSummaryEl = document.getElementById('h-risk-summary');
        riskSummaryEl.style.display = 'block';
        const dispScore = currentLang === 'ne' ? toNepaliNumerals(auditRiskScore) : auditRiskScore;
        riskSummaryEl.innerText = currentLang === 'en' ? `TOTAL RISK SCORE: ${dispScore}%` : `कुल जोखिम स्कोर: ${dispScore}%`;
        ...
        addToast("info", currentLang === 'en' ? `Generating Report for ${period}...` : `${period} को लागि प्रतिवेदन तयार गर्दै...`);
    ...
    const I18N = {
      ne: {
        govt: "नेपाल सरकार", ministry: "भौतिक पूर्वाधार तथा यातायात मन्त्रालय", dept: "सडक विभाग", city: "चाकुपाट, ललितपुर", reportTitle: "DoR प्रगति प्रतिवेदन (साप्ताहिक)",
        mainTitle: "DoR सडक विभाग", total: "कुल सूचक", met: "लक्ष्य", attention: "ध्यान", update: "अन्तिम अपडेट",
        live: "● प्रत्यक्ष", offline: "● अफलाइन", loading: "लोडिङ", search: "खोज",
        progress: "प्रगति", refreshing: "रिफ्रेस", sec: "सेकेन्ड", briefTitle: "सारांश",
        charts: "प्रगति चार्ट",
        auraText: "🤖 मद्दत?", auraAnalyzing: "जोखिम विश्लेषण... 🤖🧠", auraIsolated: "जोखिम अलग!", auraTracing: "सफलता ट्र्याक... 🤖✨", auraFiltered: "सफलता फिल्टर!",
        ...
        months: ["वैशाख", "जेठ", "असार", "साउन", "भदौ", "असोज", "कात्तिक", "मंसिर", "पुस", "माघ", "फागुन", "चैत"]
      },
      en: {
        govt: "Government of Nepal", ministry: "Ministry of Physical Infrastructure & Transport", dept: "Department of Roads", city: "Chakupat, Lalitpur", reportTitle: "DoR Progress Report (Weekly)",
        mainTitle: "DoR MIS Dashboard", total: "Total", met: "Met", attention: "Attention", update: "Updated",
        live: "● LIVE", offline: "● OFFLINE", loading: "Loading", search: "Search",
        progress: "PROGRESS", refreshing: "Refreshing", sec: "s", briefTitle: "Briefing",
        charts: "Progress Charts",
        auraText: "🤖 Help?", auraAnalyzing: "Analyzing... 🤖🧠", auraIsolated: "Isolated!", auraTracing: "Tracing... 🤖✨", auraFiltered: "Filtered!",
    ...
    document.addEventListener('mousemove', (e) => {
      ...
      aura.style.opacity = e.target.closest('button, .kpi-card, tr, .data-card') ? 1 : 0.4; // Make aura more visible on interactive elements
      ...
      if (kpi) {
        ...
        if (title.includes('attention') || title.includes('ध्यान')) {
          ...
          if (!intentTimer) intentTimer = setTimeout(() => { handleSearch('critical'); typeText(auraText, I18N[currentLang].auraIsolated); }, 1000);
          ...
          if (!intentTimer) intentTimer = setTimeout(() => { handleSearch('good'); typeText(auraText, I18N[currentLang].auraFiltered); }, 1000);
        }
      } else {
    ...
    window.resetThemeToSystem = () => {
      ...
      addToast("info", currentLang === 'en' ? "Theme reset to system default." : "थिम प्रणाली पूर्वनिर्धारितमा रिसेट गरियो।");
    };
    ...
    window.setLang = async function (l) {
      ...
      if (!window.translations) {
        ...
        } catch (e) { console.warn("Failed to fetch translations"); }
      }

      // Rapid Language Toggle Helper (with local I18N fallback)
      const t = (key, fallback) => (window.translations && window.translations[currentLang] && window.translations[currentLang][key]) || fallback || key;
      ...
      const uiMap = {
        ...
        'search-input': { prop: 'placeholder', val: t('search_placeholder', labels.search) },
        ...
        'official-pdf-btn': { prop: 'title', val: t('downloadOfficialPdf', 'Download') },
        ...
        'ai-translate-btn': { prop: 'title', val: t('translateBrief', 'Translate') },
        'ai-read-btn': { prop: 'title', val: t('readAloud', 'Narration') },
        'ai-share-audio-btn': { prop: 'title', val: t('shareAudio', 'Share') },
        'ai-download-audio-btn': { prop: 'title', val: t('downloadAudio', 'Download') },
      ...
      if (document.getElementById('lbl-year')) document.getElementById('lbl-year').innerText = labels.year;
      if (document.getElementById('lbl-month')) document.getElementById('lbl-month').innerText = labels.selectMonth;
      if (document.getElementById('lbl-diag-period')) document.getElementById('lbl-diag-period').innerText = labels.diagPeriod;
      ...
      document.getElementById('gemini-badge').innerHTML = `${labels.poweredBy} <span>Gemini</span>`;
      ...
      if (document.getElementById('ai-brief-card').style.display !== 'none') {
        document.getElementById('ai-brief-header-text').innerText = labels.briefTitle;
      }
    ...
    function setView(v) {
      currentView = v;
      document.querySelectorAll('#view-toggle .toggle-btn').forEach(b => b.classList.toggle('active', b.id === 'btn-' + v));
      ...
      document.getElementById('view-charts').classList.toggle('active-view', v === 'charts');
    ...
    function showInChartView(name) {
      ...
      setTimeout(() => {
        ...
        const el = Array.from(charts).find(c => c.getAttribute('data-indicator') === name);
        if (el) {
    ...
    function showInCardView(name) {
      ...
      setTimeout(() => {
        ...
        const el = Array.from(cards).find(c => c.getAttribute('data-indicator') === name);
        if (el) {
          el.scrollIntoView({ behavior: 'smooth', block: 'center' });
          el.classList.add('selected-row');
          setTimeout(() => el.classList.remove('selected-row'), 2000);
    ...
    function renderDropdowns() {
      ...
      mSelect.innerHTML = I18N[currentLang].months.map((m, i) =>
      ...
      ySelect.innerHTML = [currentADYear, currentADYear - 1, currentADYear - 2].map(y =>
    ...
    async function toggleHistory() {
      if (currentView === 'history') { setView('table'); return; }
      setView('history');
    ...
    function toggleHistoryTab(tab) {
      document.getElementById('hist-weekly-btn').classList.toggle('active', tab === 'weekly');
      document.getElementById('hist-cumulative-btn').classList.toggle('active', tab === 'cumulative');
      document.getElementById('cumulative-controls').style.display = tab === 'cumulative' ? 'block' : 'none';
      document.getElementById('btn-current-week').style.display = tab === 'weekly' ? 'block' : 'none';
      document.getElementById('history-list').style.display = tab === 'weekly' ? 'grid' : 'none';

      if (tab === 'cumulative') {
        renderDropdowns();
        const now = new Date();
        // Set to current Gregorian year and month, which will be converted to BS for display
        document.getElementById('summary-year').value = now.getFullYear().toString();
        document.getElementById('summary-month').value = (now.getMonth() + 1).toString().padStart(2, '0');
    ...
    async function logAnalytics(eventName, eventData = {}) {
      const payload = { eventName, eventData, url: window.location.href, time: new Date().toISOString() };

      // 1. Save to IndexedDB
      const dbRequest = indexedDB.open('dor_mis_db', 1);
      dbRequest.onupgradeneeded = (e) => e.target.result.createObjectStore('analytics', { autoIncrement: true });
    ...
    async function fetchWeeklyHistory() {
      ...
      `).join("");
      document.getElementById("history-list").innerHTML = html || "<p>No archives found yet.</p>";
    }
    ...
    async function loadCumulative(type) {
      ...
      const res = await authenticatedFetch(`${WORKER_BASE}/api/summary?type=${type}&year=${year}&month=${month}&lang=${currentLang}`);
      const json = await res.json();
      store = json;
      render(json);
      setView('table');
      document.getElementById("loader").style.display = "none";
      addToast("success", currentLang === 'en' ? `Cumulative ${type} report generated for ${period}.` : `${period} को लागि संचयी ${type} प्रतिवेदन उत्पन्न भयो।`);
    }

    window.downloadConsolidatedPdf = downloadConsolidatedPdf;
    async function downloadConsolidatedPdf() {
      const year = document.getElementById('summary-year').value;
      const type = document.getElementById('hist-weekly-btn').classList.contains('active') ? 'weekly' : 'monthly';
      const month = document.getElementById('summary-month').value;

      const res = await authenticatedFetch(`/api/consolidated-pdf?type=monthly&year=${year}&month=${month}`);
    ...
    async function downloadPdf(date) {
      ...
      try {
        ...
        if (res.ok) {
          ...
          addToast("error", currentLang === 'en' ? "Failed to download PDF archive." : "PDF अभिलेख डाउनलोड गर्न असफल भयो।");
        }
      } finally {
        if (btn) {
          btn.disabled = false;
    ...
    async function loadSnapshot(date) {
      document.getElementById("loader").style.display = "flex";
      const res = await authenticatedFetch(`/api/report?date=${date}&lang=${currentLang}`);
      const json = await res.json();
      store = json;
      render(json);
      setView('table');
    ...
    async function handleVerification() {
      ...
      setView('verify');
      document.getElementById('loader').style.display = 'flex';
      document.getElementById('verify-title').innerText = I18N[currentLang].verificationTitle;
      ...
      try {
        ...
        if (res.ok) {
          document.getElementById('verify-msg').innerText = I18N[currentLang].verifiedSuccess;
          document.getElementById('verify-msg').style.color = "var(--good)";
          document.getElementById('verify-details').innerHTML = `<b>Type:</b> ${type.toUpperCase()}<br><b>Period:</b> ${period}<br><b>Status:</b> SYSTEM_MATCH_FOUND`;
        } else {
          throw new Error();
        }
      } catch (e) {
        document.getElementById('verify-msg').innerText = I18N[currentLang].invalidReport;
      ...
      if (store && store.headers && store.headers.length > 0) {
        ...
        if (dl) {
          const matches = store.rows
            ...
            .slice(0, 10);
          dl.innerHTML = [...new Set(matches)].map(m => `<option value="${m}">`).join('');
        }
      }
    ...
    function shareApp() {
      if (navigator.share) {
        navigator.share({ title: 'DoR MIS Dashboard', text: 'Check the latest Department of Roads Progress Report.', url: window.location.href });
    ...
    function getProgress(row, headers) {
      const targetKey = headers.find(h => h.includes("Annual Target") || h.includes("बार्षिक लक्ष्य"));
      const progKey = headers.find(h => h.includes("Annual Progress") || h.includes("हाल सम्म को बार्षिक प्रगति"));
    ...
    function renderMiniChart(percent, showTrend = false) {
      const color = percent >= 80 ? 'var(--good)' : percent >= 40 ? 'var(--stable)' : 'var(--critical)';
      const trendIcon = percent >= 80 ? '<span style="color:var(--good); font-size:0.6rem; margin-left:2px;">▲</span>' : (percent < 40 ? '<span style="color:var(--critical); font-size:0.6rem; margin-left:2px;">▼</span>' : '<span style="color:var(--stable); font-size:0.6rem; margin-left:2px;">▶</span>');
    ...
    function showModal(indicatorName) {
      ...
      let details = "";
      headers.forEach((h, i) => { if (r[h]) details += `<div class="modal-item"><b>${h}</b> ${currentLang === 'ne' ? toNepaliNumerals(r[h]) : r[h]}</div>`; });
  ...
  `;
    ...
    function closeModal() {
      ...
      document.querySelectorAll('.selected-row').forEach(node => node.classList.remove('selected-row'));
    }
    ...
    window.checkForUpdates = async () => {
      ...
      addToast("info", currentLang === 'en' ? "Checking for updates..." : "अपडेट जाँच गर्दै...");
        ...
        if (reg) {
          ...
          setTimeout(() => {
            if (!reg.installing && !reg.waiting) {
              addToast("success", currentLang === 'en' ? "App is up to date." : "एप अद्यावधिक छ।");
    ...
    async function getSwChangelog() {
      ...
      return new Promise((resolve) => {
        ...
        navigator.serviceWorker.controller.postMessage({ action: 'get-changelog' }, [messageChannel.port2]);
    ...
    async function getActiveSwVersion() {
      if (!navigator.serviceWorker || !navigator.serviceWorker.controller) return 'v2.0.x';
      return new Promise((resolve) => {
        const messageChannel = new MessageChannel();
    ...
    window.showSettings = async () => {
      ...
      let voiceOptions = filteredVoices.map(v =>
          ...
          ${changelog ? `
            <div style="margin-bottom: 20px; background: var(--bg); border-radius: 12px; padding: 12px; border: 1px solid var(--border);">
              ...
              <div style="max-height: 120px; overflow-y: auto; font-size: 0.75rem; scrollbar-width: thin;">
                ${Object.entries(changelog).slice(0, 3).map(([v, changes]) => `
    ...
    window.triggerDatabaseBackup = async () => {
      ...
      try {
        ...
        const snapshot = { _meta: { timestamp: new Date().toISOString(), lang: currentLang } };
        ...
        const adminKey = prompt(currentLang === 'en' ? "Enter Admin Secret to authorize backup:" : "ब्याकअप प्रमाणित गर्न एडमिन गोप्य कुञ्जी प्रविष्ट गर्नुहोस्:");
        ...
        if (res.ok) addToast("success", currentLang === 'en' ? "Cloud backup successful!" : "क्लाउड ब्याकअप सफल भयो!");
    ...
    window.triggerDatabaseRestore = async () => {
      ...
      try {
        const adminKey = prompt(currentLang === 'en' ? "Enter Admin Secret to list backups:" : "ब्याकअप सूची हेर्न एडमिन गोप्य कुञ्जी प्रविष्ट गर्नुहोस्:");
        if (!adminKey) return;
        ...
        const listRes = await fetch(`${WORKER_BASE}/api/admin/list-backups`, { headers: { "X-Admin-Secret": adminKey } });
        const keys = await listRes.json();
        if (!keys.length) { addToast("info", "No backups found."); return; }

        const selectedKey = prompt(`Select backup to restore:\n${keys.map((k, i) => `${i + 1}. ${k.name}`).join('\n')}`, keys[0].name);
        if (!selectedKey) return;

        // 2. Download snapshot
        addToast("info", "Downloading snapshot...");
        const dataRes = await fetch(`${WORKER_BASE}/api/admin/get-backup?key=${selectedKey}`, { headers: { "X-Admin-Secret": adminKey } });
        ...
        addToast("success", currentLang === 'en' ? "Database successfully restored!" : "डाटाबेस सफलतापूर्वक रिस्टोर गरियो!");
    ...
    window.downloadAllOfflineData = async () => {
      ...
      try {
        ...
        for (const entry of snapshots) {
          await authenticatedFetch(`/api/report?date=${entry.date}&lang=${currentLang}`);
          currentStep++; updateProgress();
        }
    ...
    async function updateStorageUsageDisplay() {
      ...
      if (navigator.storage && navigator.storage.estimate) {
        ...
        const formatSize = (bytes) => {
          ...
          else if (bytes < 1048576) { val = (bytes / 1024).toFixed(1); unit = " KB"; }
          else { val = (bytes / 1048576).toFixed(2); unit = " MB"; }
        ...
        const dispPerc = currentLang === 'ne' ? toNepaliNumerals(displayPercent) : displayPercent;
        ...
        if (bar) {
          ...
          bar.style.background = percent > 80 ? 'var(--critical)' : 'var(--primary)';
        }
      } else {
        el.innerText = "N/A";
      }
    ...
    window.clearDataCache = async () => {
      if ('caches' in window) {
        ...
        addToast("success", currentLang === 'en' ? "Data cache cleared!" : "डाटा क्यास मेटाइयो!");
        loadData();
      }
    };
    ...
    window.toggleLowData = (enabled) => {
      ...
      if (enabled) {
        addToast("info", currentLang === 'en' ? "Low Data Mode active. AI Briefing disabled." : "कम डाटा मोड सक्रिय। एआई सारांश असक्षम गरियो।");
      }
    ...
    window.updateFontSize = (val) => {
      ...
      if (display) display.innerText = (currentLang === 'ne' ? toNepaliNumerals(val) : val) + 'x';
    };
    ...
    window.updateVoicePreference = (uri) => {
      ...
      const msg = new SpeechSynthesisUtterance(currentLang === 'en' ? "Voice updated." : "आवाज परिवर्तन गरियो।");
      const voices = speechSynthesis.getVoices();
    ...
    window.toggleDarkSchedule = (enabled) => {
      ...
      if (enabled) {
        navigator.geolocation.getCurrentPosition((pos) => {
          ...
          syncAppTheme();
          addToast("success", currentLang === 'en' ? "Location synced" : "स्थान सिङ्क");
        }, () => {
          addToast("info", currentLang === 'en' ? "Default schedule" : "पूर्वनिर्धारित शेड्युल");
          syncAppTheme();
        });
      } else {
        syncAppTheme();
    ...
    function showIosInstallInstructions() {
      ...
      const title = isChrome
        ? (currentLang === 'en' ? "Install via Chrome on iOS" : "iOS मा Chrome मार्फत इन्स्टल गर्नुहोस्")
        : (currentLang === 'en' ? "Install App on iPhone" : "आइफोनमा एप इन्स्टल गर्नुहोस्");

      const step1 = currentLang === 'en'
        ? `1. Tap the 'Share' icon ${isChrome ? '(at the top right)' : '(at the bottom center)'}.`
        : "१. स्क्रिनको तल रहेको 'Share' आइकनमा ट्याप गर्नुहोस्।";
      const step2 = currentLang === 'en'
    ...
    setInterval(() => {
      ...
      const t = I18N[currentLang];
      document.getElementById('refresh-timer').innerText = `(${t.refreshing} ${refreshCounter}${t.sec})`;
    }, 1000);
    ...
    window.addEventListener('touchend', () => {
      ...
      if (dist > pullThreshold) {
        ...
        indicator.innerHTML = '<span class="spinner" style="border-top-color:var(--primary); width:20px; height:20px;"></span>';
        // Mobile Force Refresh: Pulling down now handles everything including clearing Gemini cache
        loadData(true).finally(() => {
          setTimeout(() => {
            indicator.style.top = '-60px';
            indicator.classList.remove('visible');
            setTimeout(() => { indicator.innerHTML = '🔄'; }, 300);
          }, 500);
        });
      } else {
        indicator.style.top = '-60px';
    ...
    function updateConnStrength(duration) {
      ...
      else if (duration > 1200) { label = t.connFair; color = "#facc15"; } // Yellow
    ...
    async function loadData(isForced = false) {
      ...
      const skeleton = Array(10).fill(`<tr class="skeleton-row"><td><div></div></td>${Array(5).fill('<td><div></div></td>').join('')}</tr>`).join('');
      document.getElementById("tbody").innerHTML = skeleton;

      try {
        const res = await authenticatedFetch(`/api/report?lang=${currentLang}${isForced ? '&force=true' : ''}`, {
          cache: "no-store"
        });
    ...
    function render(json) {
      ...
      if (searchText && searchText !== 'verify') {
        rows = rows.filter(r => Object.values(r).some(v => String(v).toLowerCase().includes(searchText)));
      }

      // Update Results Counter
      const resCounter = document.getElementById('results-count');
      if (searchText && rows.length > 0 && resCounter) {
        const dispNum = currentLang === 'ne' ? toNepaliNumerals(rows.length) : rows.length;
      ...
      if (searchText === 'verify') {
        ...
        const rawCritical = json.rows.filter(r => r._status === 'critical').length;
        const rawRisk = rawTotal > 0 ? Math.round((rawCritical / rawTotal) * 100) : 0;
        console.log(`%c TOTAL SYSTEM RISK SCORE: ${rawRisk}% `, "background: #ef4444; color: white; font-weight: bold; padding: 4px; border-radius: 4px;");
        console.log(`Audit Summary: ${rawCritical} Critical indicators identified across ${rawTotal} total project records.`);

        const audit = rows.map(r => {
          const annTargetKey = headers.find(h => h.includes("Annual Target") || h.includes("बार्षिक लक्ष्य"));
          const annProgKey = headers.find(h => h.includes("Annual Progress") || h.includes("हाल सम्म को बार्षिक प्रगति"));
          const totTargetKey = headers.find(h => h.includes("Total Target") || h.includes("कुल लक्ष्य"));
          const totProgKey = headers.find(h => h.includes("Total Progress") || h.includes("कुल प्रगति"));
      ...
      const isNumericSearch = searchText && !isNaN(parseFloat(arabicSearch)) && isFinite(arabicSearch);
      ...
      if (currentSort.key) {
        rows.sort((a, b) => {
          ...
          return String(v1).localeCompare(String(v2), currentLang) * currentSort.dir;
        });
      }
  ...
  `;
      document.getElementById("chart-path").setAttribute("stroke-dasharray", `${percent}, 100`);
      document.getElementById("chart-percent").innerText = `${dispPerc}%`;

      if (json.lastUpdate) {
        document.getElementById("last-update").innerText = `${t.update} ${currentLang === 'ne' ? toNepaliNumerals(json.lastUpdate) : `${json.lastUpdate} BS`}`;
        document.getElementById("print-stamp").innerText = `Last Updated Version: ${json.lastUpdate}`;
      }
      const isLowData = localStorage.getItem('low-data') === 'true';
      ...
      headers.forEach((h, i) => { thead += `<th onclick="sortData('${h}'); event.stopPropagation()">${h} ${currentSort.key === h ? (currentSort.dir === 1 ? '↑' : '↓') : ''}</th>`; });
      thead += "</tr>";
      ...
      if (rows.length === 0 && searchText) {
        ...
        rows.forEach((r) => {
          ...
          headers.forEach((h, i) => {
            ...
            const color = isStatus ? (r._status === 'good' ? 'var(--good)' : r._status === 'critical' ? 'var(--critical)' : 'var(--stable)') : 'inherit';
      ...
      if (rows.length === 0) {
        ...
        rows.forEach((r) => {
          ...
          const dispAnn = currentLang === 'ne' ? toNepaliNumerals(annPerc) : annPerc;
          const dispTot = currentLang === 'ne' ? toNepaliNumerals(totPerc) : totPerc;

          const annColor = annPerc < 50 ? 'var(--critical)' : 'var(--primary)';
          const annPulseClass = annPerc < 20 ? 'pulse-critical' : '';

          let details = "";
          headers.slice(1, 6).forEach((h, i) => { if (r[h]) details += `<div style="font-size:0.75rem;margin-bottom:4px"><span style="color:var(--text-light)">${h}:</span> <span style="font-weight:600">${currentLang === 'ne' ? toNepaliNumerals(r[h]) : r[h]}</span></div>`; });
          cards += `
      ...
      if (rows.length === 0) {
        ...
        rows.forEach(r => {
          ...
          const dispProg = currentLang === 'ne' ? toNepaliNumerals(prog) : prog;
          const color = prog >= 80 ? 'var(--good)' : prog >= 40 ? 'var(--stable)' : 'var(--critical)';
          chartHtml += `
      <div class="chart-card" data-indicator="${name}" onclick="showModal('${name.replace(/'/g, "\\'")}', this, true)" style="cursor:pointer">
        ...
        <div style="font-size:0.65rem; color:var(--text-light); text-transform:uppercase; letter-spacing:0.05em">
          ${headers.slice(1, 4).map(h => `<span>${h}: <b>${currentLang === 'ne' ? toNepaliNumerals(r[h]) : r[h]}</b></span>`).join(" | ")}
        </div>
      </div>`;
        });
      }
    ...
    window.generateClientPDF = async () => {
      if (!store || !store.rows.length) return addToast("error", "No data to export");

      addToast("info", currentLang === 'en' ? "Generating PDF..." : "PDF तयार गर्दै...");
      ...
      try {
        ...
        page.drawRectangle({ x: 50, y: yOffset - 10, width: width - 100, height: 40, color: rgb(0.95, 0.95, 0.95) });
        ...
        page.drawText(kpiText, {
          x: 60, y: yOffset, size: 10, font: getFont(kpiText), color: rgb(0.2, 0.2, 0.2)
        });
        yOffset -= 60;
        ...
        headers.forEach((h, i) => {
          page.drawText(h, { x: 50 + (i * colWidth), y: yOffset, size: 9, font: getFont(h) });
        });
        ...
        store.rows.forEach((row, rowIndex) => {
          ...
          headers.forEach((h, i) => {
            ...
            page.drawText(text.substring(0, 30), {
              ...
              size: 8,
              font: getFont(text),
              color: row._status === 'critical' && i === 0 ? rgb(0.9, 0, 0) : rgb(0, 0, 0)
            });
          });
          yOffset -= 15;
        });
