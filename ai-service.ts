  ...
  const generateProjectSummary = ai.defineFlow(
    ...
    async (
      ...
      if (response.finishReason === "blocked" || !response.output) {
        console.warn(
          "Summary blocked by safety filters. Retrying with restrictive prompt...",
        );
      ...
      if (response.finishReason === "blocked" || !response.output) {
        throw new Error(
          "AI_SUMMARY_BLOCKED_AFTER_RETRY: Content triggered safety filters after retry.",
        );
      }
...
export async function runTranslation(apiKey: string, input: TranslationInput) {
  ...
  const translateFlow = ai.defineFlow(
    ...
    async (input: z.infer<typeof translationInputSchema>): Promise<string> => {
      ...
      if (response.finishReason === "blocked") {
        throw new Error(
          "AI_TRANSLATION_BLOCKED_AFTER_RETRY: Safety filters triggered even after retry.",
        );
      }
