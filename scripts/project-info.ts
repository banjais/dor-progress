...
// Main
const pkg = JSON.parse(fs.readFileSync(path.resolve(process.cwd(), 'package.json'), 'utf8')) as PackageJson;
...
try {
    ...
    console.log(`Name:         ${lines.find(l => l.trim().startsWith('name = '))?.split('=')[1]?.trim().replace(/"/g, '') || 'N/A'}`);
    console.log(`Worker:       ${lines.find(l => l.trim().startsWith('main = '))?.split('=')[1]?.trim().replace(/"/g, '') || 'index.ts'}`);
    console.log(`Compatibility: ${lines.find(l => l.trim().startsWith('compatibility_date = '))?.split('=')[1]?.trim().replace(/"/g, '') || 'N/A'}`);
} catch {
