...
const formatTimestamp = (d) => new Date(d).toLocaleString();
...
try {
    ...
    console.log(`Name:         ${lines
        ...
        ?.trim()
        .replace(/"/g, "") || "N/A"}`);
    console.log(`Worker:       ${lines
        .find((l) => l.trim().startsWith("main = "))
        ?.split("=")[1]
        ?.trim()
        .replace(/"/g, "") || "index.ts"}`);
    console.log(`Compatibility: ${lines
...
if (fs.existsSync(devVarsPath)) {
    ...
    if (parsedId)
        console.log(`SheetID: ${parsedId.substring(0, 8)}...${parsedId.substring(parsedId.length - 8)}`);
}
else {
