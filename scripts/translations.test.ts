...
describe("Translations Integrity", () => {
  ...
  it("should be valid JSON with expected top-level keys", () => {
    ...
    expect(
      translations,
      "Could not parse translations.json - verify it is valid JSON",
    ).not.toBeNull();
  ...
  it("should have parity between English and Nepali keys", () => {
    ...
    expect(
      missingInNe,
      `Missing in 'ne': ${missingInNe.join(", ")}`,
    ).toHaveLength(0);
    expect(
      missingInEn,
  ...
  it("should not exceed the character limit for values", () => {
    ...
    Object.entries({ en: translations.en, ne: translations.ne }).forEach(
      ([lang, data]: [string, any]) => {
        Object.entries(data).forEach(([key, value]) => {
          expect(
            String(value).length,
            `${lang} value for key "${key}" exceeds ${MAX_LENGTH}`,
