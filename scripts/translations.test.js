...
describe("Translations Integrity", () => {
    ...
    it("should exist at src/locales/translations.json", () => {
        expect(fileExists, 'Missing translations.json. Run "npx tsx scripts/sync-sheets.js" to sync from Google Sheets.').toBe(true);
    });
    ...
    it("should not have empty or whitespace-only values", () => {
        if (!translations)
            return;
        const { en, ne } = translations;
        Object.entries({ en, ne }).forEach(([lang, data]) => {
    ...
    it("should not exceed the character limit for values", () => {
        ...
        Object.entries({ en: translations.en, ne: translations.ne }).forEach(([lang, data]) => {
            Object.entries(data).forEach(([key, value]) => {
