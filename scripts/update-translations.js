...
try {
  ...
  execSync(
    `wrangler kv key put --namespace=${KV_NAMESPACE} "${KV_KEY}" '${translations}'`,
    { stdio: "inherit" },
  );

  console.log(
    `✅ Translations from ${TRANSLATION_FILE} successfully uploaded to KV namespace ${KV_NAMESPACE} under key "${KV_KEY}".`,
  );
} catch (error) {
  ...
  console.error(
    `   And that your 'wrangler.toml' has a KV namespace binding for '${KV_NAMESPACE}'.`,
  );
  process.exit(1);
}
